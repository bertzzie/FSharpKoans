...
module ``about lists`` =
    ...
    let BuildingNewLists() =
        ...
        //first.Head <- "apple" // Compiler error: Property 'Head' cannot be set
        //first.Tail <- ["peach"; "pear"] // Compiler error: Property 'Tail' cannot be set

        //THINK ABOUT IT: Can you change the contents of a list once it has been
        //                created?
        // USUALLY, you can. F#'s list appears to be fully immutable
    ...
    // :: is O(1) because you only need to set the next item pointer of the left list to the right
    // @  is O(n) because you can't modify first element in F# so you have to recreate the list 
